/*
  ...
  2. Security
    ...
    - Add policies for public read access to cook profiles

  3. Functions
    - Auto-create profiles when users register
    - Handle both regular users and cooks
*/
...
-- Allow authenticated users to read cook profiles for discovery
...
-- Function to automatically create profiles when users register
...
BEGIN
  -- Extract user data from metadata with smart defaults
  user_name := COALESCE(
