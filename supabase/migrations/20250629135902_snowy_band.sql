/*
  ...
  1. New Data
    - Sample cook profiles with complete information
    - Sample customer profile for testing
    - Diverse menu items from different cuisines
  
  2. Security
    - Uses proper conflict resolution with existing data
    - No direct auth.users manipulation (managed by Supabase Auth)
    - Follows database constraints and relationships
  
  3. Changes
    - Inserts sample cook and customer profiles
    - Populates menu items for each cook
    - Uses proper UUID format and data types
*/

-- Insert sample cook profiles directly
-- Note: These should correspond to users created through Supabase Auth
