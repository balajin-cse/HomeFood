/*
  # Add sample data for HomeFood application
  
  1. Sample Data
    - 6 diverse cook profiles with different cuisines
    - 18 menu items across breakfast, lunch, and dinner
    - 1 test customer profile
  
  2. Conflict Resolution
    - Delete existing sample data by email and ID
    - Use ON CONFLICT clauses to prevent constraint violations
    - Handle both unique email and ID constraints
*/

-- Clear any existing sample data first to prevent conflicts
-- Delete by email first to handle email constraint conflicts
...
-- Delete by ID to catch any remaining records
...
-- Delete menu items (foreign key constraint will handle this automatically, but being explicit)
...
-- Insert sample cook profiles with conflict handling
...
-- Insert sample menu items for each cook with conflict handling
